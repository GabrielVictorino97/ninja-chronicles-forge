...
export const battleService = {
  ...
  simulateAction(
    state: Battle,
    action: "basic" | "defend" | "item" | "flee" | "jutsu",
    jutsuName?: string,
  ): Battle {
    ...
    if (action === "basic") {
      ...
      log.push({
        ...
        turn: next.turn,
        actor: "player",
        message: `Ataque básico em ${next.enemy.name}.`,
        damage: dmg,
      });
      ...
      log.push({
        ...
        turn: next.turn,
        actor: "player",
        message: "Você assume postura defensiva.",
      });
    } else if (action === "item") {
      ...
      log.push({
        ...
        turn: next.turn,
        actor: "player",
        message: `Usou Pílula de soldado (+${heal} HP).`,
        damage: -heal,
      });
      ...
      log.push({
        ...
        turn: next.turn,
        actor: "player",
        message: `Usou ${jutsuName ?? "Jutsu"} em ${next.enemy.name}.`,
        damage: dmg,
      });
    }
    ...
    if (next.enemy.hp <= 0) {
      log.push({
        ...
        turn: next.turn,
        actor: "system",
        message: "Vitória! Você derrotou o inimigo.",
      });
      next.status = "victory";
      next.log = log;
      return next;
    ...
    log.push({
      ...
      turn: next.turn,
      actor: "enemy",
      message: `${next.enemy.name} contra-atacou.`,
      damage: enemyDmg,
    });
