    ...
    ...columns,
    {
      ...
      render: (row) => (
        <div className="flex justify-end gap-1">
          <Button size="icon" variant="ghost" onClick={() => setEdit({ ...row })}>
            <Pencil className="h-4 w-4" />
          </Button>
          <Button size="icon" variant="ghost" onClick={() => setDel(row)}>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button onClick={() => setEdit(makeEmpty())}>
      ...
      <Dialog open={!!edit} onOpenChange={(v) => !v && setEdit(null)}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {title} — {data.some((r) => r.id === edit?.id) ? "Editar" : "Criar"}
            </DialogTitle>
          </DialogHeader>
          {edit && (
                ...
                return (
                  <div key={f.name}>
                    ...
                    <Input
                      ...
                      value={String(value ?? "")}
                      onChange={(e) =>
                        update(f.type === "number" ? Number(e.target.value) : e.target.value)
                      }
                    />
                  </div>
                );
