      ...
      <DataTable<AdminUser>
        ...
        columns={[
          ...
          },
          {
            ...
            render: (u) => (
              <StatusPill
                tone={u.status === "active" ? "success" : u.status === "banned" ? "danger" : "warn"}
              >
                {u.status}
              </StatusPill>
            ),
          ...
          { key: "ip", label: "IP" },
          {
            ...
            render: (u) => (
              <div className="flex justify-end gap-1">
                <Button size="icon" variant="ghost" onClick={() => setView(u)}>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {u.status !== "banned" ? (
                      <DropdownMenuItem onClick={() => setConfirm({ user: u, type: "ban" })}>
                        <Ban className="h-4 w-4 mr-2" /> Banir
                      </DropdownMenuItem>
                    ) : (
                      <DropdownMenuItem onClick={() => setConfirm({ user: u, type: "unban" })}>
                        <Check className="h-4 w-4 mr-2" /> Desbanir
                      </DropdownMenuItem>
                    )}
                    <DropdownMenuItem onClick={() => setConfirm({ user: u, type: "block" })}>
                      <Lock className="h-4 w-4 mr-2" /> Bloquear conta
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
          ...
          {view && (
            <div className="space-y-3 text-sm">
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <span className="text-muted-foreground">Role:</span> {view.role}
                </div>
                <div>
                  <span className="text-muted-foreground">Status:</span> {view.status}
                </div>
                <div>
                  <span className="text-muted-foreground">IP:</span> {view.ip}
                </div>
              </div>
              <div>
                <h4 className="text-xs uppercase text-muted-foreground mt-3 mb-2">
                  Histórico de login
                </h4>
