...
function CharactersPage() {
  ...
  const { data = [] } = useQuery({
    ...
    queryFn: () => adminCharacterService.list(),
  });
  ...
  const [confirm, setConfirm] = useState<{ c: AdminCharacter; type: "reset" | "block" } | null>(
    null,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-2">
        <Input
          placeholder="Buscar personagem ou usuário..."
          value={search}
      ...
      <DataTable<AdminCharacter>
        ...
        columns={[
          ...
          { key: "userName", label: "Usuário" },
          {
            key: "villageId",
            label: "Vila",
            render: (c) => villageById.get(c.villageId)?.name ?? c.villageId,
          },
            ...
            render: (c) => (
              <div className="flex justify-end gap-1">
                ...
                <Button
                  ...
                  variant="ghost"
                  title="Resetar"
                  onClick={() => setConfirm({ c, type: "reset" })}
                >
                ...
                <Button
                  ...
                  variant="ghost"
                  title="Bloquear"
                  onClick={() => setConfirm({ c, type: "block" })}
                >
                  <Lock className="h-4 w-4" />
                </Button>
                <Button size="icon" variant="ghost" title="Batalhas">
                  <Swords className="h-4 w-4" />
                </Button>
                <Button size="icon" variant="ghost" title="Inventário">
      ...
      <Dialog open={!!edit} onOpenChange={(v) => !v && setEdit(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Editar atributos — {edit?.name}</DialogTitle>
          </DialogHeader>
          {edit && (
            <div className="grid grid-cols-2 gap-3">
                ...
                <div key={k}>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEdit({
                        ...edit,
                        attributes: { ...edit.attributes, [k]: Number(e.target.value) },
                      })
                    }
                  />
                </div>
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={edit.ryous}
                  onChange={(e) => setEdit({ ...edit, ryous: Number(e.target.value) })}
                />
              </div>
            </div>
          )}
