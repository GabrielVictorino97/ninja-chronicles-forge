...
function BattlesPage() {
  const { data = [] } = useQuery({ queryKey: ["admin-battles"], queryFn: () => adminBattleService.list() });
  ...
  const filtered = useMemo(() => data.filter((b) =>
    ...
    (b.player1.toLowerCase().includes(search.toLowerCase()) || b.player2.toLowerCase().includes(search.toLowerCase()))
  ), [data, search, type]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-2">
        <Input placeholder="Buscar jogador..." value={search} onChange={(e) => setSearch(e.target.value)} className="md:max-w-sm" />
      ...
      <DataTable<AdminBattle>
        ...
        columns={[
          ...
          { key: "status", label: "Status", render: (b) => <StatusPill tone={b.status === "completed" ? "success" : "warn"}>{b.status}</StatusPill> },
          { key: "actions", label: "", className: "text-right", render: (b) => (
            <Button size="icon" variant="ghost" onClick={() => setView(b)}><Eye className="h-4 w-4" /></Button>
          ) },
        ]}
      />
      ...
      <Dialog open={!!view} onOpenChange={(v) => !v && setView(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader><DialogTitle>Batalha {view?.id}</DialogTitle></DialogHeader>
          {view && (
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-2">
                <div><span className="text-muted-foreground">{view.player1}</span> vs <span className="text-muted-foreground">{view.player2}</span></div>
                <div className="text-right">Vencedor: <strong>{view.winner}</strong></div>
              </div>
              <div>
                ...
                <ul className="space-y-1 max-h-64 overflow-y-auto">
                  {view.turns.map((t) => (
                    <li key={t.n} className="flex justify-between border-b border-border/40 py-1">
                      ...
                      <span>{t.actor} usou <strong>{t.action}</strong></span>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">{view.rewards.map((r) => <StatusPill key={r} tone="info">{r}</StatusPill>)}</div>
              </div>
            </div>
          )}
        </DialogContent>
